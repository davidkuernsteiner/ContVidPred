# - This file is adapted from https://github.com/hpcaitech/Open-Sora/blob/main/opensora/datasets/utils.py
...
def temporal_random_crop(vframes, num_frames, frame_interval):
    temporal_sample = transforms.TemporalRandomCrop(num_frames * frame_interval)
...
def get_transforms_image(name="center", image_size=(256, 256)):
    if name == "center":
        ...
        transform = transforms_pt.Compose(
            [
                transforms_pt.Lambda(lambda pil_image: resize_crop_to_fill(pil_image, image_size)),
                transforms_pt.ToTensor(),
                transforms_pt.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True),
            ]
        )
...
#TAKEN FROM: https://github.com/2y7c3/Super-Resolution-Neural-Operator/blob/main/utils.py
