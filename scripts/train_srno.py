...
from vitssm.engine.tasks import SRNOEngine
...
def main(config: str):   
    ...
    with wandb.init(
        ...
        engine = SRNOEngine(model=model, run_object=run_config, resume=dit_config.resume)

        train_loader, val_loaders = get_dataloaders_srno(run_config.dataset)

        engine.train(train_loader, val_loaders)
